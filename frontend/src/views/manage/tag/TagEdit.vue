<template>
  <a-modal v-model="show" title="修改模块" @cancel="onClose" :width="450">
...
export default {
  ...
  methods: {
    setFormValues ({...tag}) {
      this.rowId = tag.id
      let fields = ['name', 'deleteFlag']
      let obj = {}
      Object.keys(tag).forEach((key) => {
        if (key === 'deleteFlag') {
          this.checked = tag[key].toString() === '0'
        }
