...
import BulletinEdit from './CustomEdit.vue'
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case '0':
              return <a-tag>未接单</a-tag>
            case '1':
              return <a-tag>已接单</a-tag>
            case '2':
              return <a-tag>定制完成</a-tag>
            case '3':
  ...
  methods: {
    pay (row) {
      let data = { outTradeNo: row.code, subject: `定制订单`, totalAmount: row.orderPrice, body: '' }
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/custom-order-info/' + ids).then(() => {
