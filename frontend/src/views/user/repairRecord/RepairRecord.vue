...
<!--        <a-button type="primary" ghost @click="add">添加订单</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-if="record.addressId != null" type="file-search" @click="orderAuditOpen(record)" title="修 改" style="margin-left: 15px"></a-icon>
          <a-icon v-if="record.addressId != null && record.status == 2" type="check" @click="orderComplete(record)" title="完 成" style="margin-left: 15px"></a-icon>
        </template>
      </a-table>
    </div>
    <order-audit
...
import OrderAudit from './RepairRecordView.vue'
import MapView from '../../manage/map/Map.vue'
moment.locale('zh-cn')

export default {
  name: 'order',
  components: {OrderAudit, RangeDate, MapView},
  data () {
    return {
      advanced: false,
      orderAdd: {
  ...
  methods: {
    orderComplete (row) {
      this.$get(`/cos/furniture-repair-record/repairComplete`, {
        'repairId': row.id
      }).then((r) => {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/furniture-repair-record/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/furniture-repair-record/page', {
        ...params
      }).then((r) => {
